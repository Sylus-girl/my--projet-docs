...
def num(x1,y1,b1,c1):   #num 函数用于绘制普通牌的牌号和花色。
    t.seth(0)
    t.penup()
    t.goto(x1+a*25,y1)
    t.pendown()
...
def wang(x1,y1,b1,c1,d1):    #wang 函数专门用于绘制大小王的标志。
    t.seth(0)
    t.penup()        #x1, y1 是文字的位置，b1 是要绘制的内容，c1 和 d1 是字体大小和颜色。
    t.goto(x1+a*25,y1)
    t.pendown()
    t.color(d1)
    t.write(b1,font=("微软雅黑",c1,"normal"))   
...
def jx1(x2,y2,b2,color1,color2):   #jx1 绘制带有颜色填充的牌框。
    t.penup()
    t.goto(x2+25*a,y2)
    t.color(color1,color2)     #x2 和 y2 是牌的位置，color1 和 color2 设置框和填充的颜色。
    t.pendown()
    t.begin_fill()
    for m in range(2):
        t.forward(b2*0.8)
...
t.color("")
t.tracer(False)
for a in range(20):    #第一段循环将 20 张牌分发给地主。
    t.color("black")
    b=r.randint(0,len(pk)-1)                  #在每次循环中，r.randint 随机抽取一张牌
    dizhu.append(pk[b])                   #存入相应玩家的牌堆（dizhu, nm1 或 nm2）
    ...
    if(pk[b]=="blackJOKER"):
        ...
        if(pk[b][-1]!="0"):
            ...
            num(-180,200,pk[b][0],11)      #，并从 pk 中删除该牌，避免重复发牌。
    del pk[b]

    
               
for a in range(17):         #后两段分别将 17 张牌发给两个农民。
    t.color("black")
